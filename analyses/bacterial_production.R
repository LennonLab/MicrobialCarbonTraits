################################################################################
#                                                                              #
#	Catechol Project: Catechol Bacterial Production Experiment                   #
...
rm(list=ls())
setwd("~/../Box Sync/Catechol Production")
se <- function(x){sd(x)/sqrt(length(x))}

# June 14 Experiment
bp_cpm <- read.delim("20140616_CatecholExperiment.txt", header=T)
...
bp_plot <- barplot(mean.bp, names.arg=trt.bp, ylim=c(0,200000), col="gray",lwd=3, yaxt="n")
arrows(x0 = bp_plot, y0 = mean.bp, y1 = mean.bp - se.bp, angle = 90, length=0.1, lwd = 2)
arrows(x0 = bp_plot, y0 = mean.bp, y1 = mean.bp + se.bp, angle = 90, length=0.1, lwd = 2)
...
# June 19 Experiment
bp_cpm <- read.delim("20140619_CatecholExperiment.txt", header=T)
...
trt.bp <- c("Kill Control", "Blank", "0 mg C/L", "0.0001 mg C/L", "0.001 mg C/L", "0.01 mg C/L", "0.1 mg C/L", "1 mg C/L")
mean.bp <- c(mean_KC, mean_Blank, mean_0mgC, mean_0.0001mgC, mean_0.001mgC, mean_0.01mgC, mean_0.1mgC, mean_1mgC) 
se.bp <- c(se_KC, se_Blank, se_0mgC, se_0.0001mgC, se_0.001mgC, se_0.01mgC, se_0.1mgC, se_1mgC)

par(mar=c(6,6,0.5,0.5), oma=c(1,1,1,1)+0.1, lwd=2)
bp_plot <- barplot(mean.bp, names.arg=trt.bp, ylim=c(0,200000), col="gray",lwd=3, yaxt="n", xaxt="n")
arrows(x0 = bp_plot, y0 = mean.bp, y1 = mean.bp - se.bp, angle = 90, length=0.1, lwd = 2)
arrows(x0 = bp_plot, y0 = mean.bp, y1 = mean.bp + se.bp, angle = 90, length=0.1, lwd = 2)
axis(side = 2, labels=T, lwd.ticks=2, las=2, lwd=2)
text(bp_plot, par("usr")[3], labels = trt.bp, srt = 45, adj = c(1.1,1.5), xpd = TRUE, cex=1)
mtext("CPM", side = 2, cex = 1.5, line = 4.5)

# Response Curve
cv.mean.bp <- c(trt_0mgC, trt_0.0001mgC, trt_0.001mgC, trt_0.01mgC, trt_0.1mgC, trt_1mgC)
cv.conc.bp <- c(0,0,0, 0.0001,0.0001,0.0001, 0.001,0.001,0.001, 0.01,0.01,0.01, 0.1,0.1,0.1, 1.0,1.0,1.0)
...
# August 1 Experiment
bp_cpm <- read.delim("20140801_BPExperiment.txt", header=T)
...
trt.bp <- c("Kill Control", "Control", "D-Glucose", "Salicylate", "Protocatechuate", "Sigma Humics", "Suwannee River")
...
par(mar=c(6,6,0.5,0.5), oma=c(1,1,1,1)+0.1, lwd=2)
bp_plot <- barplot(mean.bp, names.arg=trt.bp, ylim=c(0,75000), col="gray",lwd=3, yaxt="n", xaxt="n")
arrows(x0 = bp_plot, y0 = mean.bp, y1 = mean.bp - se.bp, angle = 90, length=0.1, lwd = 2)
arrows(x0 = bp_plot, y0 = mean.bp, y1 = mean.bp + se.bp, angle = 90, length=0.1, lwd = 2)
axis(side = 2, labels=T, lwd.ticks=2, las=2, lwd=2)
text(bp_plot, par("usr")[3], labels = trt.bp, srt = 45, adj = c(1.1,1.5), xpd = TRUE, cex=1)
